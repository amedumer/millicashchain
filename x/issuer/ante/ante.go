...
func (cicd CheckUserCredentialsDecorator) AnteHandle(
	...
	for _, msg := range tx.GetMsgs() {
		...
		case *bank.MsgSend:
			...
			fmt.Println("HERE ARE THAT ISSUERS")
			issuers := cicd.issuerk.GetAllIssuersWithCondition(ctx, []byte{0x61}, func(issuers types.Issuer) bool { return issuers.Token == msg.Amount[0].Denom })
			fmt.Println(issuers)
			...
			var vcs []vctypes.VerifiableCredential

			for _, issuer := range issuers {
				vcs = append(vcs, cicd.vcsk.GetAllVerifiableCredentialsByIssuer(ctx, issuer.IssuerDid)...)
			}
			//vcs = append(vcs, cicd.vcsk.GetAllVerifiableCredentialsByIssuer(ctx, issuer.IssuerDid)...)
...
func (cicd CheckUserCredentialsDecorator) validateUserCredential(
	...
	fmt.Println("START CHECKING FOR ADDRESS")
	fmt.Println(address)
	fmt.Println(vcs)
